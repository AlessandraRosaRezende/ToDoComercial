...
export function Dashboard() {
  ...
  const [questionMap, setQuestionMap] = useState<Record<string, boolean>>({})
      ...
      await Promise.all(
        projects.map(async p => {
          try {
            ...
            results[p.id] = res.data.data.comments?.some(
              (c: { content: string }) => c.content.startsWith('❓')
            ) ?? false
          } catch { results[p.id] = false }
        })
      )
      setQuestionMap(results)
    }
    run()
  }, [projects])
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 32px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
          <div onClick={() => setStatus('')} style={{
            ...
            color: status === '' ? 'var(--accent)' : 'var(--text2)',
          }}>
            ...
            return (
              <div key={s} onClick={() => setStatus(active ? '' : s)} style={{
                ...
                <span style={{
                  ...
                  background: active ? cfg.dot : 'var(--bg4)',
        ...
        <div style={{ marginBottom: 24, position: 'relative' }}>
          ...
          {search && (
            <button onClick={() => setSearch('')} style={{
              ...
              color: 'var(--text3)', fontSize: 16, background: 'none', border: 'none', cursor: 'pointer',
            }}>✕</button>
          )}
        </div>
        ...
        {/* Content */}
        {loading && (
          <div style={{ textAlign: 'center', padding: '80px 0', color: 'var(--text3)' }}>Carregando projetos...</div>
        )}
        {!loading && error && (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            <div style={{ color: 'var(--red)', marginBottom: 8 }}>{error}</div>
            <div style={{ fontSize: 12, color: 'var(--text3)' }}>
              Verifique se o backend está rodando: <code style={{ background: 'var(--bg3)', padding: '2px 6px', borderRadius: 4 }}>cd backend && npm run dev</code>
            </div>
          </div>
        )}
        ...
        {!loading && !error && projects.length > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 14 }}>
            {projects.map(p => (
              <ProjectCard
                key={p.id} project={p}
                hasQuestions={questionMap[p.id] ?? false}
